...
# The model is ~1.1 GB resident + per-batch activations. One encode at a time,
# because the Dockerfile now gives torch both of the box's 2 vCPUs
# (OMP_NUM_THREADS=2): a second concurrent encode would mean 4 threads on 2
# cores, which is slower in wall-clock than running them back to back. The
# single slot is what makes one document finish fast, and ingest concurrency
# in main.py still lets another document OCR while this one embeds.
...
def get_embeddings(
    ...
    Concurrency: document embeds are serialised by a process-wide semaphore so
    concurrent ingests don't run several encodes at once and OOM. Search
    queries deliberately bypass it — see below. The semaphore is blocking
    (threading) because this runs inside asyncio.to_thread.
    """
    # A query is a single short text that encodes in milliseconds. Gating it on
    # the ingest semaphore meant every chat question asked during an ingest
    # queued behind that whole document — waiting up to the 600s timeout and
    # then failing with "Embedding queue full", while the model sat idle
    # between the document's batches. One extra short encode costs negligible
    # memory, so queries skip the queue and stay fast under load.
    if kind == "query":
        return _encode_batches(texts, kind, on_progress)

    # Bound concurrent document encodes; queue behind the semaphore instead of
...
def _encode_batches(
    ...
    """Batched encode with per-batch retry. Holds no lock of its own."""
    model = _load_model()
    ...
    for start in range(0, total, batch_size):
        ...
        for attempt in range(3):
            try:
                # show_progress_bar=False avoids tqdm overhead that otherwise
                # allocates a bar per batch and logs to stderr at high
                # concurrency; normalize already true.
